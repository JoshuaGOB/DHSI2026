...
def load_settings(env=os.environ) -> Settings:
    ...
    if provider not in DEFAULT_MODELS:
        raise ValueError(
            f"LLM_PROVIDER must be 'openai', 'anthropic', or 'ollama', got {provider!r}"
        )
    embedding_provider = env.get("EMBEDDING_PROVIDER", "openai").lower()
    if embedding_provider not in DEFAULT_EMBEDDING_MODELS:
        raise ValueError(
            f"EMBEDDING_PROVIDER must be 'openai' or 'ollama', got {embedding_provider!r}"
        )
    missing = [
        key for key in ("ZOTERO_USER_ID", "ZOTERO_API_KEY") if not env.get(key)
    ]
    ...
    return Settings(
        ...
        or DEFAULT_EMBEDDING_MODELS[embedding_provider],
