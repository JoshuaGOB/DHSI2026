...
LLM + embeddings from OpenAI or Anthropic, configured by env var.
...
3. **`rag.py`** —
   - `summarize_paper(zotero_key)`: retrieve top-k chunks from that paper,
     prompt LLM for a structured summary.
...
4. **`api.py`** (FastAPI) —
   - `GET /collections`
   - `GET /collections/{id}/papers` (with `indexed: bool` per paper)
...
- `papers(zotero_key PK, title, authors, year, item_type, collection_id,
  indexed_at, chunk_count, embedding_model)`
...
- `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` — whichever the provider needs.
